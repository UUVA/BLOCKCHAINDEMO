...
const messageType = require('./message-type.js'); // 引入消息类型
const Messages = require('./message.js'); // 引入消息模块
...
class PeerToPeer {
    ...
    handleMessage(peer, message) {
        switch (message.type) {
            case REQUEST_LATEST_BLOCK:
                ...
                break;
            case REQUEST_LATEST_BLOCKCHAIN:
            ...
            case RECEIVE_LATEST_BLOCK:
                ...
                break;
            case RECEIVE_LATEST_BLOCKCHAIN:
                // 当收到整个区块链时，处理该区块链
                this.handleReceivedBlockchain(message);
                break;
            default:
                console.error('Received invalid message:', message.type);
        }
    }
    ...
    handleReceivedLatestBlock(message, peer) {
        ...
        if (latestBlock.hash === receivedBlock.previousHash) {
            try {
                ...
                console.error('Error adding received block:', err.message);
            }
    ...
    handleReceivedBlockchain(message) {
        ...
        try {
            ...
            console.error('Error replacing blockchain:', err.message);
        }
    }
}
