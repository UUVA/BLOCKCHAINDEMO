...
const p2p = new P2P(blockchain); // 创建p2p实例，传入区块链实例方便p2p模块访问区块链数据


/****************************************
 * 主函数：初始化cli命令行界面
 ****************************************/
function vopal_cli(welcome) {
    vorpal.use(connectCommand).use(discoverCommand).use(blockchainCommand).use(peersCommand).use(mineCommand).use(openCommand).use(apiCommand).delimiter('blockchain>>>>>>>').show();
    // welcome: 欢迎指令；connect: 连接节点指令，discover: 发现节点指令，blockchain: 查看区块链指令，peers: 查看已连接的节点指令，mine: 挖矿指令，open: 开启端口指令，api: 开启API接口
}
...
// * 欢迎指令，显示欢迎消息并自动显示帮助
function welcomeCommand(vorpal) {
